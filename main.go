...
func main() {
	...
	pflag.StringVarP(&flagTarget, "target", "t", "", "Target name (basename of YAML in --config-dir, e.g. 'ipa' for hosts/ipa.yaml)")
	pflag.StringVarP(&flagConfigDir, "config-dir", "c", "hosts", "Directory containing host YAML files")
	...
	pflag.StringVarP(&flagIdentity, "identity-file", "i", "", "Path to private key file (overrides config)")
	...
	v.SetDefault("config-dir", flagConfigDir)
	...
	if cfg.Host == "" {
		fatal("host is required. Set --host, SSHCLI_HOST, or provide a target via --target with host in YAML")
	}
	if cfg.User == "" {
		...
		// fatal("user is required. Set --user, SSHCLI_USER, or provide via config")
	}
	if cfg.Port <= 0 {
		cfg.Port = 22
	}
